package cloud.englert.experimental.qrgenerator

import kotlin.math.floor
import kotlin.math.log

class Version(val code: Int, val errorCorrection: ErrorCorrection, val mode: Int) {
    fun getLengthBitsNumber(): Int {
        val modeIndex = floor(log(mode.toDouble(), 2.0)).toInt()
        val bitsIndex = if (code > 26) 2
        else if (code > 9) 1
        else 0
        return LENGTH_BITS[modeIndex][bitsIndex]
    }

    fun getDataCodewordsNumber(): Int {
        return CODEWORDS[code - 1][errorCorrection.ordinal][0]
    }

    companion object {
        fun of(mode: Int, characters: Int, errorCorrection: ErrorCorrection): Version {
            val modeIndex = floor(log(mode.toDouble(), 2.0)).toInt()
            val errorIndex = errorCorrection.ordinal
            for (version in 0..39) {
                if (CAPACITIES[version][errorIndex][modeIndex] >= characters) {
                    return Version(version + 1, errorCorrection, mode)
                }
            }
            return Version(40, errorCorrection, mode)
        }

        private val CAPACITIES = arrayOf(
            // version -> error correction L-M-Q-H -> mode (num, alpha, byte, kanji)
            arrayOf( // version 1
                arrayOf(41, 25, 17, 10),
                arrayOf(34, 20, 14, 8),
                arrayOf(27, 16, 11, 7),
                arrayOf(17, 10, 7, 4)
            ), arrayOf( // version 2
                arrayOf(77, 47, 32, 20),
                arrayOf(63, 38, 26, 16),
                arrayOf(48, 29, 20, 12),
                arrayOf(34, 20, 14, 8)
            ), arrayOf( // version 3
                arrayOf(127, 77, 53, 32),
                arrayOf(101, 61, 42, 26),
                arrayOf(77, 47, 32, 20),
                arrayOf(58, 35, 24, 15)
            ), arrayOf( // version 4
                arrayOf(187, 114, 78, 48),
                arrayOf(149, 90, 62, 38),
                arrayOf(111, 67, 46, 28),
                arrayOf(82, 50, 34, 21)
            ), arrayOf( // version 5
                arrayOf(255, 154, 106, 65),
                arrayOf(202, 122, 84, 52),
                arrayOf(144, 87, 60, 37),
                arrayOf(106, 64, 44, 27)
            ), arrayOf( // version 6
                arrayOf(322, 195, 134, 82),
                arrayOf(255, 154, 106, 65),
                arrayOf(178, 108, 74, 45),
                arrayOf(139, 84, 58, 36)
            ), arrayOf( // version 7
                arrayOf(370, 224, 154, 95),
                arrayOf(293, 178, 122, 75),
                arrayOf(207, 125, 86, 53),
                arrayOf(154, 93, 64, 39)
            ), arrayOf( // version 8
                arrayOf(461, 279, 192, 118),
                arrayOf(365, 221, 152, 93),
                arrayOf(259, 157, 108, 66),
                arrayOf(202, 122, 84, 52)
            ), arrayOf( // version 9
                arrayOf(552, 335, 230, 141),
                arrayOf(432, 262, 180, 111),
                arrayOf(312, 189, 130, 80),
                arrayOf(235, 143, 98, 60)
            ), arrayOf( // version 10
                arrayOf(652, 395, 271, 167),
                arrayOf(513, 311, 213, 131),
                arrayOf(364, 221, 151, 93),
                arrayOf(288, 174, 119, 74)
            ), arrayOf( // version 11
                arrayOf(772, 468, 321, 198),
                arrayOf(604, 366, 251, 155),
                arrayOf(427, 259, 177, 109),
                arrayOf(331, 200, 137, 85)
            ), arrayOf( // version 12
                arrayOf(883, 535, 367, 226),
                arrayOf(691, 419, 287, 177),
                arrayOf(489, 296, 203, 125),
                arrayOf(374, 227, 155, 96)
            ), arrayOf( // version 13
                arrayOf(1022, 619, 425, 262),
                arrayOf(796, 483, 331, 204),
                arrayOf(580, 352, 241, 149),
                arrayOf(427, 259, 177, 109)
            ), arrayOf( // version 14
                arrayOf(1101, 667, 458, 282),
                arrayOf(871, 528, 362, 223),
                arrayOf(621, 376, 258, 159),
                arrayOf(468, 283, 194, 120)
            ), arrayOf( // version 15
                arrayOf(1250, 758, 520, 320),
                arrayOf(991, 600, 412, 254),
                arrayOf(703, 426, 292, 180),
                arrayOf(530, 321, 220, 136)
            ), arrayOf( // version 16
                arrayOf(1408, 854, 586, 361),
                arrayOf(1082, 656, 450, 277),
                arrayOf(775, 470, 322, 198),
                arrayOf(602, 365, 250, 154)
            ), arrayOf( // version 17
                arrayOf(1548, 938, 644, 397),
                arrayOf(1212, 734, 504, 310),
                arrayOf(876, 531, 364, 224),
                arrayOf(674, 408, 280, 173)
            ), arrayOf( // version 18
                arrayOf(1725, 1046, 718, 442),
                arrayOf(1346, 816, 560, 345),
                arrayOf(948, 574, 394, 243),
                arrayOf(746, 452, 310, 191)
            ), arrayOf( // version 19
                arrayOf(1903, 1153, 792, 488),
                arrayOf(1500, 909, 624, 384),
                arrayOf(1063, 644, 442, 272),
                arrayOf(813, 493, 338, 208)
            ), arrayOf( // version 20
                arrayOf(2061, 1249, 858, 528),
                arrayOf(1600, 970, 666, 410),
                arrayOf(1159, 702, 482, 297),
                arrayOf(919, 557, 382, 235)
            ), arrayOf( // version 21
                arrayOf(2232, 1352, 929, 572),
                arrayOf(1708, 1035, 711, 438),
                arrayOf(1224, 742, 509, 314),
                arrayOf(969, 587, 403, 248)
            ), arrayOf( // version 22
                arrayOf(2409, 1460, 1003, 618),
                arrayOf(1872, 1134, 779, 480),
                arrayOf(1358, 823, 565, 348),
                arrayOf(1056, 640, 439, 270)
            ), arrayOf( // version 23
                arrayOf(2620, 1588, 1091, 672),
                arrayOf(2059, 1248, 857, 528),
                arrayOf(1468, 890, 611, 376),
                arrayOf(1108, 672, 461, 284)
            ), arrayOf( // version 24
                arrayOf(2812, 1704, 1171, 721),
                arrayOf(2188, 1326, 911, 561),
                arrayOf(1588, 963, 661, 407),
                arrayOf(1228, 744, 511, 315)
            ), arrayOf( // version 25
                arrayOf(3057, 1853, 1273, 784),
                arrayOf(2395, 1451, 997, 614),
                arrayOf(1718, 1041, 715, 440),
                arrayOf(1286, 779, 535, 330)
            ), arrayOf( // version 26
                arrayOf(3283, 1990, 1367, 842),
                arrayOf(2544, 1542, 1059, 652),
                arrayOf(1804, 1094, 751, 462),
                arrayOf(1425, 864, 593, 365)
            ), arrayOf( // version 27
                arrayOf(3517, 2132, 1465, 902),
                arrayOf(2701, 1637, 1125, 692),
                arrayOf(1933, 1172, 805, 496),
                arrayOf(1501, 910, 625, 385)
            ), arrayOf( // version 28
                arrayOf(3669, 2223, 1528, 940),
                arrayOf(2857, 1732, 1190, 732),
                arrayOf(2085, 1263, 868, 534),
                arrayOf(1581, 958, 658, 405)
            ), arrayOf( // version 29
                arrayOf(3909, 2369, 1628, 1002),
                arrayOf(3035, 1839, 1264, 778),
                arrayOf(2181, 1322, 908, 559),
                arrayOf(1677, 1016, 698, 430)
            ), arrayOf( // version 30
                arrayOf(4158, 2520, 1732, 1066),
                arrayOf(3289, 1994, 1370, 843),
                arrayOf(2358, 1429, 982, 604),
                arrayOf(1782, 1080, 742, 457)
            ), arrayOf( // version 31
                arrayOf(4417, 2677, 1840, 1132),
                arrayOf(3486, 2113, 1452, 894),
                arrayOf(2473, 1499, 1030, 634),
                arrayOf(1897, 1150, 790, 486)
            ), arrayOf( // version 32
                arrayOf(4686, 2840, 1952, 1201),
                arrayOf(3693, 2238, 1538, 947),
                arrayOf(2670, 1618, 1112, 684),
                arrayOf(2022, 1226, 842, 518)
            ), arrayOf( // version 33
                arrayOf(4965, 3009, 2068, 1273),
                arrayOf(3909, 2369, 1628, 1002),
                arrayOf(2805, 1700, 1168, 719),
                arrayOf(2157, 1307, 898, 553)
            ), arrayOf( // version 34
                arrayOf(5253, 3183, 2188, 1347),
                arrayOf(4134, 2506, 1722, 1060),
                arrayOf(2949, 1787, 1228, 756),
                arrayOf(2301, 1394, 958, 590)
            ), arrayOf( // version 35
                arrayOf(5529, 3351, 2303, 1417),
                arrayOf(4343, 2632, 1809, 1113),
                arrayOf(3081, 1867, 1283, 790),
                arrayOf(2361, 1431, 983, 605)
            ), arrayOf( // version 36
                arrayOf(5836, 3537, 2431, 1496),
                arrayOf(4588, 2780, 1911, 1176),
                arrayOf(3244, 1966, 1351, 832),
                arrayOf(2524, 1530, 1051, 647)
            ), arrayOf( // version 37
                arrayOf(6153, 3729, 2563, 1577),
                arrayOf(4775, 2894, 1989, 1224),
                arrayOf(3417, 2071, 1423, 876),
                arrayOf(2625, 1591, 1093, 673)
            ), arrayOf( // version 38
                arrayOf(6479, 3927, 2699, 1661),
                arrayOf(5039, 3054, 2099, 1292),
                arrayOf(3599, 2181, 1499, 923),
                arrayOf(2735, 1658, 1139, 701)
            ), arrayOf( // version 39
                arrayOf(6743, 4087, 2809, 1729),
                arrayOf(5313, 3220, 2213, 1362),
                arrayOf(3791, 2298, 1579, 972),
                arrayOf(2927, 1774, 1219, 750)
            ), arrayOf( // version 40
                arrayOf(7089, 4296, 2953, 1817),
                arrayOf(5596, 3391, 2331, 1435),
                arrayOf(3993, 2420, 1663, 1024),
                arrayOf(3057, 1852, 1273, 784)
            )
        )

        private val CODEWORDS = arrayOf(
            // version -> ec level L-M-Q-H ->
            // dc, ecc/block, blocks/group1, dc/block (group1), blocks/group2, dc/block (group2)
            arrayOf( // version 1
                arrayOf(19,  7, 1, 19, 0, 0),
                arrayOf(16, 10, 1, 16, 0, 0),
                arrayOf(13, 13, 1, 13, 0, 0),
                arrayOf( 9, 17, 1,  9, 0, 0)
            ), arrayOf( // version 2
                arrayOf(34, 10, 1, 34, 0, 0),
                arrayOf(28, 16, 1, 28, 0, 0),
                arrayOf(22, 22, 1, 22, 0, 0),
                arrayOf(16, 28, 1, 16, 0, 0)
            ), arrayOf( // version 3
                arrayOf(55, 15, 1, 55, 0, 0),
                arrayOf(44, 26, 1, 44, 0, 0),
                arrayOf(34, 18, 2, 17, 0, 0),
                arrayOf(26, 22, 2, 13, 0, 0)
            ), arrayOf( // version 4
                arrayOf(80, 20, 1, 80, 0, 0),
                arrayOf(64, 18, 2, 32, 0, 0),
                arrayOf(48, 26, 2, 24, 0, 0),
                arrayOf(36, 16, 4,  9, 0, 0)
            ), arrayOf( // version 5
                arrayOf(108, 26, 1, 108, 0,  0),
                arrayOf( 86, 24, 2,  43, 0,  0),
                arrayOf( 62, 18, 2,  15, 2, 16),
                arrayOf( 46, 22, 2,  11, 2, 12)
            ), arrayOf( // version 6
                arrayOf(136, 18, 2, 68, 0, 0),
                arrayOf(108, 16, 4, 27, 0, 0),
                arrayOf( 76, 24, 4, 19, 0, 0),
                arrayOf( 60, 28, 4, 15, 0, 0)
            ), arrayOf( // version 7
                arrayOf(156, 20, 2, 78, 0,  0),
                arrayOf(124, 18, 4, 31, 0,  0),
                arrayOf( 88, 18, 2, 14, 4, 15),
                arrayOf( 66, 26, 4, 13, 1, 14)
            ), arrayOf( // version 8
                arrayOf(194, 24, 2, 97, 0,  0),
                arrayOf(154, 22, 2, 38, 2, 39),
                arrayOf(110, 22, 4, 18, 2, 19),
                arrayOf( 86, 26, 4, 14, 2, 15)
            ), arrayOf( // version 9
                arrayOf(232, 30, 2, 116, 0,  0),
                arrayOf(182, 22, 3,  36, 2, 37),
                arrayOf(132, 20, 4,  16, 4, 17),
                arrayOf(100, 24, 4,  12, 4, 13)
            ), arrayOf( // version 10
                arrayOf(274, 18, 2, 68, 2, 69),
                arrayOf(216, 26, 4, 43, 1, 44),
                arrayOf(154, 24, 6, 19, 2, 20),
                arrayOf(122, 28, 6, 15, 2, 16)
            ), arrayOf( // version 11
                arrayOf(324, 20, 4, 81, 0,  0),
                arrayOf(254, 30, 1, 50, 4, 51),
                arrayOf(180, 28, 4, 22, 4, 23),
                arrayOf(140, 24, 3, 12, 8, 13)
            ), arrayOf( // version 12
                arrayOf(370, 24, 2, 92, 2, 93),
                arrayOf(290, 22, 6, 36, 2, 37),
                arrayOf(206, 26, 4, 20, 6, 21),
                arrayOf(158, 28, 7, 14, 4, 15)
            ), arrayOf( // version 13
                arrayOf(428, 26,  4, 107, 0,  0),
                arrayOf(334, 22,  8,  37, 1, 38),
                arrayOf(244, 24,  8,  20, 4, 21),
                arrayOf(180, 22, 12,  11, 4, 12)
            ), arrayOf( // version 14
                arrayOf(461, 30,  3, 115, 1, 116),
                arrayOf(365, 24,  4,  40, 5,  41),
                arrayOf(261, 20, 11,  16, 5,  17),
                arrayOf(197, 24, 11,  12, 5,  13)
            ), arrayOf( // version 15
                arrayOf(523, 22,  5, 87, 1, 88),
                arrayOf(415, 24,  5, 41, 5, 42),
                arrayOf(295, 30,  5, 24, 7, 25),
                arrayOf(223, 24, 11, 12, 7, 13)
            ), arrayOf( // version 16
                arrayOf(589, 24,  5, 98,  1, 99),
                arrayOf(453, 28,  7, 45,  3, 46),
                arrayOf(325, 24, 15, 19,  2, 20),
                arrayOf(253, 30,  3, 15, 13, 16)
            ), arrayOf( // version 17
                arrayOf(647, 28,  1, 107,  5, 108),
                arrayOf(507, 28, 10,  46,  1,  47),
                arrayOf(367, 28,  1,  22, 15,  23),
                arrayOf(283, 28,  2,  14, 17,  15)
            ), arrayOf( // version 18
                arrayOf(721, 30,  5, 120,  1, 121),
                arrayOf(563, 26,  9,  43,  4,  44),
                arrayOf(397, 28, 17,  22,  1,  23),
                arrayOf(313, 28,  2,  14, 19,  15)
            ), arrayOf( // version 19
                arrayOf(795, 28,  3, 113,  4, 114),
                arrayOf(627, 26,  3,  44, 11,  45),
                arrayOf(445, 26, 17,  21,  4,  22),
                arrayOf(341, 26,  9,  13, 16,  14)
            ), arrayOf( // version 20
                arrayOf(861, 28,  3, 107,  5, 108),
                arrayOf(669, 26,  3,  41, 13,  42),
                arrayOf(485, 30, 15,  24,  5,  25),
                arrayOf(385, 28, 15,  15, 10,  16)
            ), arrayOf( // version 21
                arrayOf(932, 28,  4, 116, 4, 117),
                arrayOf(714, 26, 17,  42, 0,   0),
                arrayOf(512, 28, 17,  22, 6,  23),
                arrayOf(406, 30, 19,  16, 6,  17)
            ), arrayOf( // version 22
                arrayOf(1006, 28,  2, 111,  7, 112),
                arrayOf( 782, 28, 17,  46,  0,   0),
                arrayOf( 568, 30,  7,  24, 16,  25),
                arrayOf( 442, 24, 34,  13,  0,   0)
            ), arrayOf( // version 23
                arrayOf(1094, 30,  4, 121,  5, 122),
                arrayOf( 860, 28,  4,  47, 14,  48),
                arrayOf( 614, 30, 11,  24, 14,  25),
                arrayOf( 464, 30, 16,  15, 14,  16)
            ), arrayOf( // version 24
                arrayOf(1174, 30,  6, 117,  4, 118),
                arrayOf( 914, 28,  6,  45, 14,  46),
                arrayOf( 664, 30, 11,  24, 16,  25),
                arrayOf( 514, 30, 30,  16,  2,  17)
            ), arrayOf( // version 25
                arrayOf(1276, 26,  8, 106,  4, 107),
                arrayOf(1000, 28,  8,  47, 13,  48),
                arrayOf( 718, 30,  7,  24, 22,  25),
                arrayOf( 538, 30, 22,  15, 13,  16)
            ), arrayOf( // version 26
                arrayOf(1370, 28, 10, 114, 2, 115),
                arrayOf(1062, 28, 19,  46, 4,  47),
                arrayOf( 754, 28, 28,  22, 6,  23),
                arrayOf( 596, 30, 33,  16, 4,  17)
            ), arrayOf( // version 27
                arrayOf(1468, 30,  8, 122,  4, 123),
                arrayOf(1128, 28, 22,  45,  3,  46),
                arrayOf( 808, 30,  8,  23, 26,  24),
                arrayOf( 628, 30, 12,  15, 28,  16)
            ), arrayOf( // version 28
                arrayOf(1531, 30,  3, 117, 10, 118),
                arrayOf(1193, 28,  3,  45, 23,  46),
                arrayOf( 871, 30,  4,  24, 31,  25),
                arrayOf( 661, 30, 11,  15, 31,  16)
            ), arrayOf( // version 29
                arrayOf(1631, 30,  7, 116,  7, 117),
                arrayOf(1267, 28, 21,  45,  7,  46),
                arrayOf( 911, 30,  1,  23, 37,  24),
                arrayOf( 701, 30, 19,  15, 26,  16)
            ), arrayOf( // version 30
                arrayOf(1735, 30,  5, 115, 10, 116),
                arrayOf(1373, 28, 19,  47, 10,  48),
                arrayOf( 985, 30, 15,  24, 25,  25),
                arrayOf( 745, 30, 23,  15, 25,  16)
            ), arrayOf( // version 31
                arrayOf(1843, 30, 13, 115,  3, 116),
                arrayOf(1455, 28,  2,  46, 29,  47),
                arrayOf(1033, 30, 42,  24,  1,  25),
                arrayOf( 793, 30, 23,  15, 28,  16)
            ), arrayOf( // version 32
                arrayOf(1955, 30, 17, 115,  0,  0),
                arrayOf(1541, 28, 10,  46, 23, 47),
                arrayOf(1115, 30, 10,  24, 35, 25),
                arrayOf( 845, 30, 19,  15, 35, 16)
            ), arrayOf( // version 33
                arrayOf(2071, 30, 17, 115,  1, 116),
                arrayOf(1631, 28, 14,  46, 21,  47),
                arrayOf(1171, 30, 29,  24, 19,  25),
                arrayOf( 901, 30, 11,  15, 46,  16)
            ), arrayOf( // version 34
                arrayOf(2191, 30, 13, 115,  6, 116),
                arrayOf(1725, 28, 14,  46, 23,  47),
                arrayOf(1231, 30, 44,  24,  7,  25),
                arrayOf( 961, 30, 59,  16,  1,  17)
            ), arrayOf( // version 35
                arrayOf(2306, 30, 12, 121,  7, 122),
                arrayOf(1812, 28, 12,  47, 26,  48),
                arrayOf(1286, 30, 39,  24, 14,  25),
                arrayOf( 986, 30, 22,  15, 41,  16)
            ), arrayOf( // version 36
                arrayOf(2434, 30,  6, 121, 14, 122),
                arrayOf(1914, 28,  6,  47, 34,  48),
                arrayOf(1354, 30, 46,  24, 10,  25),
                arrayOf(1054, 30,  2,  15, 64,  16)
            ), arrayOf( // version 37
                arrayOf(2566, 30, 17, 122,  4, 123),
                arrayOf(1992, 28, 29,  46, 14,  47),
                arrayOf(1426, 30, 49,  24, 10,  25),
                arrayOf(1096, 30, 24,  15, 46,  16)
            ), arrayOf( // version 38
                arrayOf(2702, 30,  4, 122, 18, 123),
                arrayOf(2102, 28, 13,  46, 32,  47),
                arrayOf(1502, 30, 48,  24, 14,  25),
                arrayOf(1142, 30, 42,  15, 32,  16)
            ), arrayOf( // version 39
                arrayOf(2812, 30, 20, 117,  4, 118),
                arrayOf(2216, 28, 40,  47,  7,  48),
                arrayOf(1582, 30, 43,  24, 22,  25),
                arrayOf(1222, 30, 10,  15, 67,  16)
            ), arrayOf( // version 40
                arrayOf(2956, 30, 19, 118,  6, 119),
                arrayOf(2334, 28, 18,  47, 31,  48),
                arrayOf(1666, 30, 34,  24, 34,  25),
                arrayOf(1276, 30, 20,  15, 61,  16)
            )
        )

        private val LENGTH_BITS = arrayOf(
            // mode (num, alpha, byte, kanji) -> version code (1-9, 10-26, 27-40)
            arrayOf(10, 12, 14),
            arrayOf(9, 11, 13),
            arrayOf(8, 16, 16),
            arrayOf(8, 10, 12)
        )
    }

    enum class ErrorCorrection {
        LOW, MEDIUM, QUARTILE, HIGH
    }
}